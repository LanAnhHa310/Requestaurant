...
async function openPopup(r) {
  ...
  // Build a Google Maps embed URL from the restaurant's location.
  // Use both name + location to make the search more accurate.
  if (r.location) {
    const mapQuery = encodeURIComponent(`${r.name}, ${r.location}`);

    // This works for an <iframe> embed without needing an API key
    restaurantMap.src = `https://www.google.com/maps?q=${mapQuery}&output=embed`;

    // Show a human-readable address under the map
    restaurantAddress.textContent = `Address: ${r.location}`;
    restaurantAddress.classList.remove("hidden");
  } else {
    // If there is no location, hide the map + address
    restaurantMap.classList.add("hidden");
    restaurantAddress.classList.add("hidden");
  }
